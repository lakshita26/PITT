"""
PITT — Lid-Driven Cavity with Semicircular Obstacle
====================================================
Changes in this version:
  • GRID_SIZE = 81  (max stable at DT=0.001, Re=400)
  • OBSTACLE_RADIUS = 0.4 * L_DOM
  • Each plot saved as a SEPARATE file
  • Shared colorbar range (vmin/vmax) across all spatial plots
  • Training progress logged every 250 epochs
  • Loss curve uses linear x-axis with epoch ticks every 250
  • Plot 1 uses 'turbo' colormap for pressure to match reference.
  • Plot 2 displays pressure field with velocity streamlines mapped to 'Greens'.
"""
...
def build_obstacle_mask(grid_size=GRID_SIZE, radius=OBSTACLE_RADIUS,
    ...
    return ((X - cx)**2 + Y**2 <= radius**2) & (Y <= radius)
...
def generate_cfd_data():
    print(f"CFD: grid={GRID_SIZE}x{GRID_SIZE}, Re={REYNOLDS}, R={OBSTACLE_RADIUS:.2f}")
...
def obs_patch(ax, alpha=1.0):
    """Draw filled semicircle obstacle on axes."""
    theta = np.linspace(0, np.pi, 400)
    cx    = 0.5 * L_DOM
    px    = np.concatenate([cx + OBSTACLE_RADIUS*np.cos(theta),
                            [cx+OBSTACLE_RADIUS, cx-OBSTACLE_RADIUS]])
    py    = np.concatenate([OBSTACLE_RADIUS*np.sin(theta), [0.0, 0.0]])
    ax.fill(px, py, color='#d0d0d0', zorder=5, alpha=alpha)
    ax.plot(cx + OBSTACLE_RADIUS*np.cos(theta),
            OBSTACLE_RADIUS*np.sin(theta),
            color='#444444', lw=1.0, zorder=6)
...
def evaluate_and_plot(model, history, eval_input):
    ...
    # CHANGED: Switched to 'turbo' colormap to match image 1.
    ...
    fig, ax = plt.subplots(figsize=(7, 6), facecolor='white')
    # CHANGED: Replaced speed background with pressure (pm) using 'coolwarm'
    cf = ax.contourf(X, Y, pm, levels=LEVELS_P, cmap='coolwarm',
                     vmin=p_vmin, vmax=p_vmax, extend='both')
    # CHANGED: Colored streamlines by speed using 'Greens' colormap to replicate the dark green lines at the top.
    ...
    # shade obstacle region on profile
    y_obs_max = OBSTACLE_RADIUS
    ax.axhspan(0, y_obs_max, alpha=0.10, color='gray', label='obstacle region')
