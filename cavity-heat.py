...
def apply_bc_fast(u, v, p, T, mask, contrib, dirs, count):
    ...
    # Obstacle surface held at T_COLD
...
def generate_cfd_data():
    ...
    # Temperature: initialise linearly from T_COLD (bottom) to T_HOT (top)
    ...
    for step in range(CFD_STEPS):
        ...
        # Temperature BCs
        T[-1, :] = T_HOT    # lid (top wall) — hot
        T[0,  :] = T_COLD   # bottom wall   — cold
        T[:, 0]  = T[:, 1]  # left wall     — zero-gradient (Neumann)
        T[:,-1]  = T[:,-2]  # right wall    — zero-gradient (Neumann)
        u, v, p, T = apply_bc_fast(u, v, p, T, obstacle, contrib, dirs, count)
        # ─────────────────────────────────────────────────────────────────────
...
def tokenize_equation(nu_val=1.0/REYNOLDS, Re=REYNOLDS, bc_type='Dirichlet', pad_len=SEQ_LEN):
    ...
    # Heat equation tokens: ∂T/∂t + u·∇T = α·ΔT
    seq += [TOKEN_VOCAB['&'],
...
def train_model(dataset):
    ...
    for ep in range(EPOCHS):
        ...
        for i in range(0,N,BATCH_SIZE):
            ...
            up,vp = pred[:,0], pred[:,1]
            Tp    = pred[:,2]   # channel 2 = T
            # divergence-free penalty
            div = ((up[:,1:-1,2:]-up[:,1:-1,:-2])/(2*DX)+
                   (vp[:,2:,1:-1]-vp[:,:-2,1:-1])/(2*DY))
            # heat residual: ∂T/∂t ≈ (T_pred − T_in)/DT
            # convection + diffusion should match the time tendency
            Tin  = bx[:,2]        # T channel of input frame (index 2)
...
def evaluate_and_plot(model, history, eval_input):
    ...
    ax.semilogy(epochs_arr, history, color='#1a5fa8', lw=1.8, label='Total Loss (MAE + div + heat)')
    ...
    # ── Plot 10: Temperature contourf + velocity quiver ───────────────────────
    fig, ax = plt.subplots(figsize=(7, 6), facecolor='white')
    ...
    ax.quiver(X[::skip,::skip], Y[::skip,::skip],
              ...
              color='white', scale=15, width=0.003, alpha=0.85, zorder=4)
    obs_patch(ax)
    add_cbar(fig, ax, cf, 'Temperature [T*]')
    axis_style(ax, f'Temperature Field + Quiver  (Re={REYNOLDS}, Pr={PRANDTL})')
    plt.tight_layout()
    plt.savefig(f"{OUT_DIR}/10_temperature_quiver.png", dpi=150, bbox_inches='tight')
    plt.close(); print("Saved 10_temperature_quiver.png")

    # ── Plot 11: Temperature contourf + streamlines ───────────────────────────
    fig, ax = plt.subplots(figsize=(7, 6), facecolor='white')
    ...
    obs_patch(ax)
    add_cbar(fig, ax, cf, 'Temperature [T*]')
    axis_style(ax, 'Temperature Field + Velocity Streamlines')
    plt.tight_layout()
    ...
    # ── Plot 12: Temperature gradient magnitude ───────────────────────────────
    ...
    obs_patch(ax)
    add_cbar(fig, ax, cf, '|∇T| [T*/m]')
    axis_style(ax, 'Temperature Gradient Magnitude + −∇T Streamlines')
    plt.tight_layout()
    ...
    ax.axhspan(0, TRI_HEIGHT, alpha=0.10, color='gray', label='obstacle region')
    ax.set_xlabel('T [T*]', fontsize=10); ax.set_ylabel('y [m]', fontsize=10)
    ax.set_title('Temperature Profile at x = 0.5 m', fontsize=11)
    ax.yaxis.set_major_locator(MultipleLocator(0.1))
    ax.grid(alpha=0.3); ax.legend(fontsize=9)
    plt.tight_layout()
    ...
    # ── Plot 14: Coupled T–Speed scatter (every 5th fluid point) ─────────────
    T_flat = Tm.ravel()
    s_flat = speed.ravel()
    ...
    cb.set_label('Temperature [T*]', fontsize=9); cb.ax.tick_params(labelsize=8)
    ax.set_xlabel('Temperature T [T*]', fontsize=10)
    ax.set_ylabel('Speed |u| [m/s]', fontsize=10)
    ax.set_title('Temperature vs Velocity Magnitude', fontsize=11)
    ax.grid(alpha=0.3)
    plt.tight_layout()
